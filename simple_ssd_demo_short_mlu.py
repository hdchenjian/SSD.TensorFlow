...
def main(_):
    with tf.Graph().as_default():
        ...
        with tf.name_scope('define_input'):
            image_input = tf.placeholder(tf.float32, shape=(1, 300, 300, 3), name='image_input')
        print('image_input', image_input)
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[image_input], reuse=tf.AUTO_REUSE):
        ...
        with tf.Session() as sess:
            ...
            image = cv2.resize(np_image, (FLAGS.train_image_size, FLAGS.train_image_size))
            ...
            saver.save(sess, 'model/ssd300_vgg16/ssd300_vgg16_short', global_step=0)
