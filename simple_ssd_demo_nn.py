...
def main(_):
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            backbone = ssd_net_nn.VGG16Backbone(
                features, FLAGS.num_classes, all_num_anchors_depth, FLAGS.data_format)
            location_pred = backbone.loc_preds
            cls_pred = backbone.cls_preds
            if FLAGS.data_format == 'channels_first':
                cls_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in cls_pred]
                location_pred = [tf.transpose(pred, [0, 2, 3, 1]) for pred in location_pred]

            for pred in cls_pred:
                print('pred', pred, backbone.num_classes, backbone.num_anchors_depth_per_layer)
            for pred in location_pred:
                print('location_pred', pred)
