...
class VGG16Backbone(object):
    def __init__(self, _input_data, _num_classes, _num_anchors_depth_per_layer, _data_format='channels_first'):
        ...
        for ind, feat in enumerate(feature_layers):
            _loc, _cls = get_predication(ind, feat, feature_layers_pre_channel_size[ind])
    ...
    def conv_block(self, bottom, num_blocks, filters, kernel_size, pre_channel_size, strides, name):
        with tf.variable_scope(name):
            _input = bottom
            for ind in range(1, num_blocks + 1):
                w = weight_variable([kernel_size, kernel_size, pre_channel_size, filters])
                b = weight_variable([filters])
                conv = tf.nn.relu(tf.nn.conv2d(
                    _input, w, strides, padding='same', data_format=self.data_format,
    ...
    def ssd_conv_block(self, bottom, filters, pre_channel_size, strides, name, padding='same'):
        with tf.variable_scope(name):
            w = weight_variable([1, 1, pre_channel_size, filters])
            b = weight_variable([filters])
            conv = tf.nn.relu(tf.nn.conv2d(
                bottom, w, [1, 1, 1, 1], padding=padding, data_format=self.data_format,
                name='{}_{}'.format(name, 1)) + b)

            w2 = weight_variable([3, 3, pre_channel_size, filters * 2])
            b2 = weight_variable([filters * 2])
    ...
    def get_predication(ind, feat, pre_channel_size):
        with tf.variable_scope('multibox_head'):
            w = weight_variable([3, 3, pre_channel_size, num_anchors_depth_per_layer[ind] * 4])
            b = weight_variable([num_anchors_depth_per_layer[ind] * 4])
            loc = tf.nn.conv2d(feat, w, [1, 1, 1, 1], padding='same', data_format=self.data_format,
                                name='loc_{}'.format(ind)) + b

            w2 = weight_variable([3, 3, pre_channel_size, num_anchors_depth_per_layer[ind] * self.num_classes])
            b2 = weight_variable([num_anchors_depth_per_layer[ind] * self.num_classes])
            cls = tf.nn.conv2d(feat, w, [1, 1, 1, 1], padding='same', data_format=self.data_format,
                                name='cls_{}'.format(ind)) + b
            return  loc, cls
